 ...
 */
async function slideToPng(slide, options = {}) {
    ...
    try {
        ...
        // container.
        reactDom.flushSync(() => {
            root.render(react.createElement(SlideCanvas.SlideCanvas, { slide, chrome: false }));
        });
        ...
        await Promise.all(Array.from(container.querySelectorAll('video')).map((video) => replaceVideoWithFrame(video)));
        ...
        await Promise.all(Array.from(container.querySelectorAll('img')).map((img) => bakeImageFilter(img)));
        ...
        await Promise.all(Array.from(container.querySelectorAll('img[data-soft-edge]')).map((img) => bakeImageSoftEdge(img)));
 ...
 */
async function replaceVideoWithFrame(video) {
    const parent = video.parentElement;
    if (!parent)
        return;
    let imgSrc = video.poster || video.getAttribute('poster') || null;
 ...
 */
async function bakeImageFilter(img) {
 ...
 */
async function bakeImageSoftEdge(img) {
    ...
    return new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
            if (blob)
                ...
                reject(new Error('slideToPng: canvas.toBlob returned null'));
        }, 'image/png');
    });
}

exports.slideToPng = slideToPng;
